  ...
  <div class="container-scroller">
    ...
    <?= $this->include('admin/admin/template')?>
      ...
      <div class="main-panel">
                ...
                <div class="card-body">
                  ...
                  <div class="table-responsive">
                    <table class="table table-striped table-borderless">
                      ...
                      <tbody>
                        <?php foreach($funds as $data):?>
                          <tr>
                            <td class="py-1" id="event">
                                  <img src="<?= base_url('upload/funds/').$data['fund_cover']?>" alt="image"/>
                            </td>
                            <td><?= $data['fund_title']?></td>
                            <td>
                              <p><?= $data['fund_desc']?></p>
                            </td>
                            <td><?= $data['fund_enddate']?></td>
                            <td><?= $data['fund_contactperson']?></td>
                            <td><?= $data['fund_contactinfo']?></td>
                            <?php if($data['status'] == 'active'):?>
                            <td class="font-weight-medium"><div class="badge badge-success"><?= $data['status']?></div></td>
                            <?php endif;?>
                            <td>
                              <a href="#edit" data-toggle="modal" class="edit-fund"
                              data-fund-id = "<?= $data['id']?>"
                              data-fund-title = "<?= $data['fund_title']?>"
                              data-fund-enddate = "<?= $data['fund_enddate']?>"
                              data-fund-desc = "<?= $data['fund_desc']?>"
                              data-fund-contact = "<?= $data['fund_contactperson']?>"
                              data-fund-contactInfo = "<?= $data['fund_contactinfo']?>"
                              >
                              <button type="button" class="btn btn-dark btn-icon">
                                <i class="fa fa-edit btn-icon-append"></i>
                              </button>
                              </a>                               
                              <a href="#delete" data-toggle="modal" data-fund-id="<?= $data['id']?>" class="delete-fund">
                                <button type="button" class="btn btn-dark btn-icon">
        ...
        <!--Modals-->
        <!--Edit -->
        <form action="<?= base_url('AdminController/AdminFolder/fundController/updateFund')?>" method="post" enctype="multipart/form-data">
        ...
        <!--Delete-->
        <form action="<?= base_url('AdminController/AdminFolder/fundController/deletefund')?>" method="post">
        ...
        <!--Add News-->
        <form action="<?= base_url('AdminController/AdminFolder/fundController/storeFund')?>" method="post" enctype="multipart/form-data">
        <div id="add-project" class="modal fade">
          <div class="modal-dialog modal-confirm">
            <div class="modal-content">
              <div class="header">					
                <h4 class="modal-title w-100">Add Fundraising</h4>	
                        <button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
              </div>
              <div class="modal-body">
                    ...
                    <table class="table table-striped table-borderless">
                      ...
                      <tbody>
                        <?php foreach ($archi as $data): ?>
                          <tr>
                            <td class="py-1">
                            <img src="<?=base_url('upload/funds/').$data['fund_cover']; ?>" alt="">
                            </td>
                            <td><?= $data['fund_title']; ?></td>
                            <td>
                            <?= $data['status']; ?>
                            ...
                            <td>
                              <a href="<?= base_url('AdminController/AdminFolder/fundController/restorefund/'.$data['id'])?>" class="restore-job">
